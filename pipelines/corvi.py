"""Train the base Corvi detector (ResNet-50, real vs. LDM fakes).

This produces the pretrained checkpoint that DEAR-c is built on. Most users can
...
@hydra.main(config_path="../configs", config_name="corvi", version_base=None)
def main(args):
    set_seed(args.seed)
    ...
    detector = CorviDetector(
        lr=args.lr, beta1=args.beta1, weight_decay=args.weight_decay, device=args.device,
    )
